...
class TreeGenerator:
    ...
    def generate_random_tree(self, depth: int, leaf_node_chance: int = 0.3, single_operator_chance: int = 0.3, variable_chance = 0.5) -> Node:
        import random
        ...
        if random.random() < single_operator_chance:
            ...
            return Node(random_operator, self.generate_random_tree(depth - 1), None)
        else:
            random_operator = random.choice(list(self.multi_operators.keys()))
            return Node(random_operator, self.generate_random_tree(depth - 1), self.generate_random_tree(depth - 1))
